...
function transmute($raw){
  ...
  $raw = max(0, min(50, (int)$raw));
  return $map[$raw];
}

function require_login(){ ensure_session(); if(empty($_SESSION['user'])) redirect('login.php'); }
function require_role($role){ ensure_session(); if(empty($_SESSION['user'])||$_SESSION['user']['role']!==$role) redirect('login.php'); }
function is_role($role){ ensure_session(); return !empty($_SESSION['user']) && $_SESSION['user']['role']===$role; }
?>
